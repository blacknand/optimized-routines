...
static void inline __attribute ((always_inline))
memset_medium_set_16 (const char *name, void *(*set)(void *, int, size_t))
{
  printf ("%22s ", name);
  size_t alignment_size = sizeof memset_align_freq / sizeof memset_align_freq[0];
  for (size_t i = 0; i < alignment_size; i++) {
    uint8_t align = memset_align_freq[i].align;
...
int main (void)
{
  ...
  DOTEST ("Medium memset (set_16) (bytes/ns):\n", memset_medium_set_16);
